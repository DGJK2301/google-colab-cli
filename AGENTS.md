...
## Auth Recovery v1 Contracts

- **Two auth layers remain distinct:** `colab login` manages the local OAuth2
  credentials used by the CLI control plane. `colab auth` injects unrelated
  credentials into an already-running VM.
- **Refresh before replacement:** normal commands and `colab login` reuse the
  cached refresh/RAPT state. `colab login --force` is the explicit interactive
  replacement path; never instruct users to delete `token.json` first.
- **Durable cache:** credential replacement is same-directory, flushed,
  permission-restricted, and atomic. A failed consent or write must leave the
  previous cache intact.
- **Diagnosable automation:** structured session/status output identifies
  required human reauthentication as `AUTH_REAUTH_REQUIRED`; it must not call
  that condition a generic network outage or expose credential values.
...
24. **Use a cost-tiered live-validation ladder**: Validate local packaging and unit behavior first, then CLI authentication and GET-only session listing, then a free CPU runtime, and only then a paid accelerator explicitly required by the workload. Never allocate G4, A100, or another paid accelerator to debug Windows imports, CLI flags, notebook selection, dependency installation, or Drive path setup. Before accelerator allocation, the exact repository revision, project bootstrap, data/Drive contract, and minimal CPU smoke must already pass. Track every allocated endpoint, stop it in `finally`, and finish with `colab sessions` plus orphan cleanup. Encoded 2026-07-19 after the Windows fork workflow required a low-cost proof path before XoFTR G4 training.
25. **Treat Copilot review as review input, not a merge gate substitute**: Before merging a PR for which Copilot review was requested, fetch thread-aware review state and address every actionable unresolved comment. A quota/error-only Copilot review contains no code findings and must be recorded as unavailable rather than reported as a successful review; complete the repository's local review and test gates regardless. Encoded 2026-07-19 after a requested fork review returned only a Copilot quota-limit message.
26. **Prefer local independent review for this fork workflow**: After existing GitHub review threads are addressed, use multiple read-only local review agents plus the repository test and lint gates instead of repeatedly requesting Copilot or Codex remote reviews. Do not request another remote bot review unless the user explicitly asks for one. Remote CI and any already-posted human or bot comments still need to be checked before merge. Encoded 2026-07-19 after the Windows fork established a reproducible local multi-review workflow.
